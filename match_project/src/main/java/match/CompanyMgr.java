...
public class CompanyMgr {
	...
	public CompanyBean getCompanyLocation(int company_idx) {
		...
		try {
			// Assuming you have a similar connection pooling mechanism as in the previous
			// method
			con = pool.getConnection();
	...
	public boolean checkBookmarkExists(String user_id, int posting_idx) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		boolean exists = false; // 존재 여부를 저장할 변수
		String sql = "SELECT 1 FROM bookmark_posting WHERE user_id = ? AND posting_idx = ?";
		try {
			con = pool.getConnection(); // 데이터베이스 커넥션 풀에서 커넥션을 가져옴
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, user_id);
			pstmt.setInt(2, posting_idx);
			rs = pstmt.executeQuery();

			exists = rs.next(); // 결과 집합에서 다음 행이 있으면 true (존재함), 없으면 false (존재하지 않음)
		} catch (SQLException e) {
			e.printStackTrace();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			pool.freeConnection(con, pstmt, rs); // 사용한 리소스 반환
		}
		return exists; // 최종 결과 반환
	}
