...
public class ApplicationMgr2 {

	private DBConnectionMgr pool;

	public ApplicationMgr2() {
		pool = DBConnectionMgr.getInstance();
	}

	// 특정 관리자가 등록한 모든 공고에 대한 각각의 지원 이력서(resume_idx) 수를 ArrayList로 반환하는 Mgr
	public ArrayList<Integer> ResumeNum(String posting_status) {
		Connection con = null;
		...
		try {
			...
			String sql = "SELECT p.posting_idx FROM posting p WHERE p.posting_status = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, posting_status);
	...
	public void insertApplication(int posting_idx, int resume_idx, String user_id) {
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		try {
			con = pool.getConnection();
			sql = "insert application values(null, ?, ?, ?, ?, ?, ?, now(), now())";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, posting_idx);
			pstmt.setInt(2, resume_idx);
			pstmt.setString(3, user_id);
			pstmt.setString(4, "normal");
			pstmt.setString(5, "no");
			pstmt.setString(6, "미분류");
			pstmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public void insertApplicationResult(int posting_idx, String user_id) {
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		try {
			con = pool.getConnection();
			sql = "insert application_result values(null, ?, ?, ?, ?)";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, posting_idx);
			pstmt.setInt(2, 0);
			pstmt.setString(3, user_id);
			pstmt.setString(4, "심사중");
			pstmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
