...
<%
// categoryMgr 객체 생성
    certificate_categoryMgr mgr = new certificate_categoryMgr();
    // 스킬 이름 리스트 가져오기
    Vector<certificate_categoryBean> skillList = cMgr.certificatenameList();

    // 스킬 이름을 저장할 배열 생성
    String[] data = new String[skillList.size()];
    for (int i = 0; i < skillList.size(); i++) {
        data[i] = skillList.get(i).getCertificate_name().toLowerCase();
    }
%>
