					    	...
					    	<div class="row filter-btn-group" style="width: 1400px">
					            ...
					            <div class="row px-0 mx-0 search-form2" style="height: 40px;">
					            	...
					            	<div class="col form-content4" style="flex-grow:0; height: 40px;">
					            		<button type="button" id="submitFilter" class="form-btn2" style="height: 40px; font-size: 13px;">조건 검색</button>
		...
		<script>
			...
			document.addEventListener('DOMContentLoaded', function () {
			    document.getElementById('submitFilter').addEventListener('click', function () {
			        ...
			        // 환경 유형 데이터 수집
			        document.querySelectorAll('#job-category21 .scategory2.active').forEach(function(button) {
			            env_types.push(button.textContent.trim());
			        });

			        // 교육 유형 데이터 수집
			        document.querySelectorAll('#job-category19 .scategory2.active').forEach(function(button) {
			            qual_edu_types.push(button.textContent.trim());
			        });

			        // 경력 유형 데이터 수집
			        document.querySelectorAll('#job-category18 .scategory2.active').forEach(function(button) {
			            qual_experience.push(button.textContent.trim());
			        });

			        var data = JSON.stringify({
			            env_types: env_types,
			            qual_edu_types: qual_edu_types,
			            qual_experience: qual_experience
			        });

			        console.log("전송 데이터: ", data); // 전송 데이터 로그
			        sendFilterData(data);
			    });

			    function sendFilterData(data) {
			        var xhr = new XMLHttpRequest();
			        xhr.open("POST", "/match_project/GetUserEmails2Servlet", true);
